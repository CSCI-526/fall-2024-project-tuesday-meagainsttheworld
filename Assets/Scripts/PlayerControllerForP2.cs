...
public class PlayerControllerForP2 : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        IsGrounded = touchingCol.Cast(Vector2.up, castFilter, groundHits, groundDistance) > 0;
        IsOnWall = wallCheckCollider.Cast(wallCheckDirection, castFilter, wallHits, wallDistance) > 0;

        if (IsGrounded && flag == 0) playerRb.velocity = new Vector2(moveInput.x * moveSpeed, 0);
        if (IsGrounded) flag = 0;
    }
    ...
    public void OnJump(InputAction.CallbackContext context)
    {
        ...
        if (context.started && IsGrounded || (touchingCol.Cast(Vector2.down, castFilter, groundHits, groundDistance) > 0))
        {
            flag = 1;
            playerRb.velocity = new Vector2(playerRb.velocity.x, jumpImpulse * playerRb.gravityScale / (-4));
        }
    }
